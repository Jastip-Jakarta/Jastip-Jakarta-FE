import { IRegion } from "@/utils/apis/region-code/types";

const CardRegionCodeAdmin = ({ region, ...props }: { region: IRegion; [propName: string]: any }) => {
  return (
    <div className="px-4 py-3 bg-white rounded-md space-y-2 text-sm cursor-pointer" {...props}>
      <h2 className="font-semibold text-xl">{region.code}</h2>
      <div className="flex flex-row font-medium gap-x-3">
        <span className="font-semibold min-w-36">Wilayah</span>
        <span className="">:</span>
        <span>{region.region}</span>
      </div>
      <div className="flex flex-row font-medium gap-x-3">
        <span className="font-semibold min-w-36">Alamat</span>
        <span className="">:</span>
        <span>{region.full_address}</span>
      </div>
      <div className="flex flex-row font-medium gap-x-3">
        <span className="font-semibold min-w-36">Admin perwakilan</span>
        <span className="">:</span>
        <span>{region.admin_id}</span>
      </div>
      <div className="flex flex-row font-medium gap-x-3">
        <span className="font-semibold min-w-36">Nomor whatsapp</span>
        <span className="">:</span>
        <span>+{region.phone_number}</span>
      </div>
      <div className="flex flex-row font-medium gap-x-3">
        <span className="font-semibold min-w-36">Harga kode wilayah</span>
        <span className="">:</span>
        <span>Rp. {region.price.toLocaleString("id-ID")}</span>
      </div>
    </div>
  );
};

export default CardRegionCodeAdmin;
